...
bool explodeAny(Node *root, int depth, bool stop);
...
int main() {
    ...
    string fp; fin.ignore(1);
    getline(fin, fp);
    Node *sum = newFishNumber(fp);
    sum->parent = nullptr;
    topNode = sum;
    while (!fin.eof()) {
        fin.ignore(1);
        getline(fin, fp);
        sum = addFishNumbers(sum, newFishNumber(fp));
        topNode = sum;
        bool end = false;
        while (!end) {
            if (explodeAny(sum, 0)) {
    ...
    cout << "Magnitude: " << magnitude(sum) << endl;
}
